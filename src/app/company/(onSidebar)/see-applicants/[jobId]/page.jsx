          ...
          appRes.data.map(async (app) => {
            ...
            return {
              ...
              profile: {
                ...
                certificationFilePaths: jsRes.data.certificationFilePaths || [],
              },
            };
          })
        );
  ...
  const onCreateChat = async (jobSeekerId) => {
    ...
    try {
      const response = await request.post("/conversations", {
        ...
        initialMessage: "Halo, saya tertarik untuk berdiskusi mengenai lamaran Anda"
      });
    ...
    <div className="p-6 space-y-6 mt-12">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-card p-6 rounded-xl  border border-border/60">
          ...
          <p className="text-text-secondary text-sm">Kandidat diterima</p>
        </div>

        <div className="bg-card p-6 rounded-xl  border border-border/60">
          <p className="text-2xl font-bold">
            {job.quota ? job.quota - applicants.length : "-"}
          </p>
          <p className="text-text-secondary text-sm">Sisa pekerja</p>
        </div>
      </div>

      <div className="bg-card rounded-xl  border border-border/60 overflow-hidden">
        <div className="p-4 flex justify-between items-center  border border-border/60-b">
          <h2 className="font-semibold">Kandidat Pekerja / {job.title}</h2>
          {Array.isArray(applicants) && applicants.length !== 0 && (
            <Button className="btn-primary">Buka data kandidat diterima</Button>
          )}
        </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {currentData.map((a, idx) => (
              <tr
                key={a.applicationId}
                className=" border border-border/60-t hover:bg-primary-50/50 transition"
              >
        ...
        <div className="flex justify-end p-4  border border-border/60-t">
          <Pagination>
            <PaginationContent>
              <PaginationItem>
                <PaginationPrevious
